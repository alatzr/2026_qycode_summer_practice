# Day 13 教师讲义：字符串哈希、等价类与前后缀

适用班级：已完成 Day1（字符串扫描）与 Day8-12（图论/并查集）的学生。

课时定位：字符串第一课。核心是**多项式哈希**（O(1) 比较任意子串）与由此衍生的"等价类、border、前后缀"问题。标题里的"Trie 替代"指：本阶段用哈希充当字符串等价工具，Trie 的系统实现在 Day24 现教。
...
| 时间 | 内容 | 教师重点 |
|---:|---|---|
| 0-25 分钟 | 多项式哈希模板 | 前缀哈希 + 幂，O(1) 取子串 |
| 25-45 分钟 | 1791D 前后缀计数（热身） | 前后缀各扫一遍 |
| 45-70 分钟 | 126B border + 中间出现 | 哈希判前后缀相等 |
| 70-95 分钟 | 2010C2 重叠自匹配 | 枚举原串长度 |
| 95-120 分钟 | 559B 递归等价类 | 标准形归并 |
| 120-150 分钟 | 316G1 子串去重计数 | 哈希 + set |
| 150-180 分钟 | 1849C 等价键去重 | 核心区间 |
