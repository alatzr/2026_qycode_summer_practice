# Day 10 教师讲义：拓扑排序、DAG 上的 DP 与功能图找环

适用班级：已完成 Day8（BFS/Dijkstra）与 Day9（树上 DFS）的学生。

课时定位：图论第三课，聚焦三条主线——**拓扑排序模板**、**DAG 上做 DP**、**功能图（每点一条出边）找环**。Floyd 全源最短路移至假期自习包 E 模块，期望类题目不在本期主线。
...
| 时间 | 内容 | 教师重点 |
|---:|---|---|
| 0-15 分钟 | 复盘 Day9 树 DFS；引出"图上有依赖怎么办" | 树是特殊 DAG |
| 15-45 分钟 | 拓扑排序模板（入度队列版）+ 手动模拟 | 为什么队列空了还有点没出队 = 有环 |
| 45-70 分钟 | 1020B + 功能图定义 | 每点一条出边 ⇒ 每个连通块恰一个环 |
| 70-105 分钟 | 510C 约束建图 | 相邻串比较、前缀陷阱 |
| 105-140 分钟 | 919D DAG DP | "前驱全部转移完再结算自己" |
| 140-165 分钟 | 1572A 轮次代价 | 边上的条件转成 +0/+1 |
| 165-180 分钟 | 1027D 找环 + 1851E 留作下午自练引导 | 三色标记 |
| 下午 | 六题训练 + 讲评 | 按题单顺序 |
