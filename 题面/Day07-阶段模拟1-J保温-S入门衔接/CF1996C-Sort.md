...
- 难度：1200
- 标签：dp、greedy、sortings、strings
- 链接：https://codeforces.com/problemset/problem/1996/C
- 时间限制：5 seconds　内存限制：256 megabytes
- 出现位置：Day07-阶段模拟1-J保温-S入门衔接

## 中文题意

给定两个长度均为 $n$ 的字符串 $a$、$b$，以及 $q$ 次询问。每次询问给出区间 $[l,r]$：你可以任意重排 $a[l..r]$ 内部的字符（等价于对该子串排序），问最少修改 $a[l..r]$ 中多少个字符，才能使重排后的 $a[l..r]$ 与重排后的 $b[l..r]$ 相同。各询问相互独立。
...
每组第一行两个整数 $n$、$q$（$1 \le n,q \le 2\cdot10^5$）——串长与询问数。

第二行字符串 $a$（长 $n$，仅小写字母）。第三行字符串 $b$（长 $n$，仅小写字母）。

接下来 $q$ 行，每行两个整数 $l$、$r$（$1 \le l \le r \le n$）。

保证所有数据的 $n$ 之和与 $q$ 之和均不超过 $2\cdot10^5$。

## 输出格式（中文）

对每次询问输出一行一个整数，即所需的最少修改次数。
