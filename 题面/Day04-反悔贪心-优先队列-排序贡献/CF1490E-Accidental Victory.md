...
Berland 举办一场锦标赛，共有 $n$ 名选手参加，第 $i$ 名选手拥有 $a_i$（$a_i \ge 1$）枚代币。锦标赛共进行 $n-1$ 场比赛，规则如下：

- 每场比赛从当前代币数非零的选手中随机选出两人；
- 代币多者获胜（若相等则随机决定胜者）；
- 胜者夺走败者的全部代币。

最后唯一一名代币非零的选手即为总冠军。所有随机选择均等概率且相互独立。评委希望提前知道哪些选手**有可能**夺冠（即夺冠概率大于零）。请找出所有这样的选手。

## 输入格式（中文）

第一行一个整数 $t$（$1 \le t \le 10^4$）——测试数据组数。每组数据：第一行一个正整数 $n$（$1 \le n \le 2 \cdot 10^5$）——选手数；第二行 $n$ 个正整数 $a_1, a_2, \ldots, a_n$（$1 \le a_i \le 10^9$）——各选手的代币数。保证所有测试数据的 $n$ 之和不超过 $2 \cdot 10^5$。

## 输出格式（中文）

对每组数据，第一行输出有可能夺冠的选手人数；第二行按**编号升序**输出这些选手的编号（选手按输入顺序从 $1$ 开始编号）。
