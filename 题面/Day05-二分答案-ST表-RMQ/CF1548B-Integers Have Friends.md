...
- 难度：1800
- 标签：binary search、data structures、divide and conquer、math、number theory、two pointers
- 链接：https://codeforces.com/problemset/problem/1548/B
- 时间限制：2 seconds　内存限制：256 megabytes
- 出现位置：Day05-二分答案-ST表-RMQ

## 中文题意

给定一个由**互不相同**的正整数组成的数组 $a$。

称连续子数组 $a_i, a_{i+1}, \ldots, a_j$ 是一个「朋友组」，当且仅当存在整数 $m \ge 2$，使得 $a_i \bmod m = a_{i+1} \bmod m = \ldots = a_j \bmod m$（即这段数模 $m$ 全部同余）。
...
第一行一个整数 $t$（$1 \le t \le 2\cdot10^4$）——测试数据组数。

每组数据第一行一个整数 $n$（$1 \le n \le 2\cdot10^5$）——数组长度。

第二行 $n$ 个正整数 $a_1, \ldots, a_n$（$1 \le a_i \le 10^{18}$），保证互不相同。

保证所有测试数据的 $n$ 之和小于 $2\cdot10^5$。

## 输出格式（中文）

输出 $t$ 行，每行一个整数，为对应数组中最大朋友组的长度。
